...
# by an executed witness, or a lattice contradiction); 0 otherwise.
...
defmodule SpecToDescr do
  ...
  defp builtin(:map, [], _e, _d), do: {open_map(), true}
...
defmodule Main do
  ...
  defp render(results, opts) do
    ...
    out =
      case opts[:format] do
        "report" -> render_report(stats, proven, residue, all)
    ...
    # the run for CI purposes.
    contradictions = Enum.filter(residue, &(&1.verdict == :contradiction))
    if proven != [] or contradictions != [], do: System.halt(1)
  end
  ...
  defp render_report(stats, proven, residue, all) do
    total = length(all)
    ...
    residue_section =
      for e <- Enum.sort_by(residue, &verdict_rank/1) do
        slice_notes =
          e.slices
